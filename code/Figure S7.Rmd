...
pdf(file = "../figures/FigureS7A.pdf", width = 6, height = 6)
p <- ggplot(data_scatter %>%
              dplyr::arrange(desc(signature)),
            aes(x = ad_COVID, y = ad_GBS,
  ...
  xlab(label = "AD Statistic (COVID)") +
  ylab(label = "AD Statistic (GBS)") +
  coord_fixed() +
  scale_y_continuous(limits = c(-1.3, 4.8), breaks = -1:5) +
...
p <- ggplot(data_plot,
  ...
  facet_wrap2(
    signature~Assay, scales = "free_y", ncol = 7,
...
ggsave(p_fixed, filename = "../figures/FigureS7B.pdf", width = 14, height = 11)
...
# Create individual lists with the required number of repetitions
rect_list_1 <- rep(list(element_rect(fill = "#457cd6")), 1)
rect_list_2 <- rep(list(element_rect(fill = "#e34262")), 1)
rect_list_3 <- rep(list(element_rect(fill = "#945F9C")), 2)
rect_list_4 <- rep(list(element_rect(fill = "white")), 1000) ### For the markers
...
p <- ggplot(data_plot) +
  facet_wrap2(
    signature~Assay, ncol = 9, scales = "free_y",
    strip = strip_nested(bleed = FALSE, background_x = background_fill),
  ) +
  geom_boxplot(aes(x = Group, y = scaled_NPX, fill = Group), outlier.alpha = 0) +
  geom_jitter(aes(x = Group, y = scaled_NPX), size = 0.5, height = 0, width = 0.2) +
  scale_fill_manual(values = cp_group[2:4]) +
  ylab(label = "normalized protein abundance") +
  guides(fill=guide_legend(title = "Patient Group")) +
  theme_bw() +
...
ggsave(p_fixed, filename = "../figures/FigureS7C.pdf", width = 14, height = 11)
