...
lower_first <- function(x) {
  ...
  x <- as.character(x)
  nonempty <- nzchar(x)
  # for each non‑empty element, lowercase its first char in place
...
custom_corr_plot <- function(data_plot, data_plot_text, y_axis, marker){
  ...
  p <- ggplot(data_plot, aes(x = scaled_NPX, y = .data[[y_axis]], color = Group, fill = Group)) +
    ...
    scale_color_manual(values = cp_group[2]) + # removed 3
    scale_fill_manual(values = cp_group[2]) + # removed 3
    theme_bw()
...
pdf(file = "../figures/FigureS8A.pdf", width = 6, height = 3)
custom_corr_plots(
  data = data_csf,
  dual_run_patients = dual_run_patients_csf,
  marker = marker
)
invisible(dev.off())
