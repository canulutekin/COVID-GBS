...
p <- ggplot(data_plot, aes(x = get(PC_x), y = get(PC_y))) +
  ...
  # ggtitle(label = "CSF") +
  scale_color_manual(values = cp_group[1:2]) +
...
pdf(file = "../figures/Figure1C.pdf", width = 12, height = 12)
for (i in levels(marker_class$General.Classification)) {
  ...
  p <- ggplot(data_heatmap_temp,
    ...
    scale_fill_viridis(option = "B", limits = c(-1, 1)) +
...
rm(degrees, i, n_rows, n_markers, p, data_heatmap,
   data_heatmap_temp, data_heatmap_temp_axis) # cleanup
```
...
data_boxplot_stat <- data_boxplot %>%
  ...
  dplyr::ungroup() %>%
  dplyr::filter(pval_corrected <= 0.05)
...
rect_list_1 <- rep(list(element_rect(fill = "#5D9A3C")), 0) ### Signaling Molecules
...
p <- ggplot(data_plot) +
  ...
  scale_fill_manual(values = cp_group[1:3]) +
...
ggsave(p_fixed, filename = "../figures/Figure1D.pdf", width = 8, height = 5)
...
pdf(file = "../figures/Figure1E.pdf", width = 10)
print(p)
print(q)
invisible(dev.off())
...
pdf(file = "../figures/Figure1F.pdf", width = 12, height = 12)
for (i in levels(marker_class$General.Classification)) {
  data_heatmap_temp <- data_heatmap %>%
    dplyr::filter(General.Classification == i) %>%
    dplyr::group_by(Group) %>%
...
ggsave(p_fixed, filename = "../figures/Figure1G.pdf", width = 8, height = 5)
